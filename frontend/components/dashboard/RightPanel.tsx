...
export default function RightPanel({
  ...
  return (
    <aside className="hidden xl:flex fixed right-0 top-0 h-full w-[var(--right-panel-w)] bg-secondary border-l border-border p-5 flex-col gap-8 overflow-y-auto z-30"
    ...
    >
          ...
          {transactions.length === 0 ? (
            ...
            transactions.map((tx) => {
              ...
              return (
                ...
                >
                  <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", dotColor(tx.transaction_type, tx.category))} />
                  <div className="flex-1 min-w-0">
                    <h4 className="text-[12px] font-medium text-foreground truncate">
                      {tx.description}
                    </h4>
      ...
      {/* Bank summary */}
      {totalCount > 0 && (
        <section className="p-4 bg-background rounded-xl ghost-border space-y-2">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          {/* Active bank */}
          <div className="p-3 bg-background rounded-xl ghost-border flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-[#002e67] flex items-center justify-center">
                <span className="text-white font-bold text-[8px]">CHILE</span>
              </div>
              <div>
                <p className="text-[12px] font-medium text-foreground">Banco de Chile</p>
                <p className="text-[9px] text-ch-green font-bold uppercase">Conectado</p>
              </div>
            </div>
          </div>
          ...
          {SUPPORTED_BANKS.slice(1, 3).map((bank) => (
            <div
              key={bank.id}
              className="p-3 bg-background/50 rounded-xl border border-dashed border-border flex items-center gap-2.5 opacity-50"
            >
              <div className="w-8 h-8 rounded-lg bg-secondary flex items-center justify-center">
                <span className="text-muted-foreground text-[10px] font-bold">{bank.logoInitials}</span>
              </div>
              <div>
                <p className="text-[12px] font-medium text-muted-foreground">{bank.name}</p>
                <p className="text-[9px] text-muted-foreground">Próximamente</p>
              </div>
            </div>
          ))}
        </div>
